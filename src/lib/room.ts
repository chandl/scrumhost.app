...
export async function setVotingFlag(roomId: string, enableVoting: boolean) {
    try {
        ...
        const record = await pb.collection('rooms').update(roomId, 
            {...currentRoomData, is_voting_period: enableVoting});
...
export async function setActiveStory(roomId: string, storyId: string) {
    try {
        const currentRoomData = await getRoom(roomId);
        const record = await pb.collection('rooms').update(roomId, {...currentRoomData, active_story: storyId});
